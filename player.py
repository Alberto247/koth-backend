...
class RemotePlayer():
    ...
    async def send_tick(self, tick):
        data = pickle.dumps(tick)  # FIXME  why is this not a json? you can extend the json encoder if needed
        ...
        try:
            ...
            if("start" in move and len(move["start"]) == 3 and "end" in move and len(move["end"]) == 3):
                return ((move["start"][0], move["start"][1], move["start"][2]), (move["end"][0], move["end"][1], move["end"][2]))  # FIXME make shure to check types, maybe cast to int
            return ((0, 0, 0), (0, 0, 0))
        except Exception:
            return ((0, 0, 0), (0, 0, 0))
