...
def update_images():
    for x in range(PLAYERS):
        print(client.images.pull(f"team{x+1}.registry.rising0.com:7394/bot/bot:latest", auth_config={"username":f"team{x+1}", "password":passwords[x]}))

def handle_game(players, history, scoreboard, prefix):
    player_images={}
    print("Creating player containers")
    for player in players:
        player_images[player]=client.containers.run(f"team{player}.registry.rising0.com:7394/bot/bot:latest", name=f"{prefix}-bot-team{player}", environment=[f"name=team{player}"], hostname=f"player{player}", network=f"koth-client{player}", mem_limit="2g", nano_cpus=2000000000, detach=True) #TODO: limits
