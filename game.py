...
class Game:
    ...
    def __init__(self):
        ...
        # force players to have at least some distance in between one eachother TODO: This is slow but probably ok
    ...
    def reachability_fix(self, reachable_spots):
        # TODO: maybe we can avoid copy if later on we reset all the map owning, this is not too slow however
        mapcopy = copy.deepcopy(self.map)
        start = self.player_spawns[0]
    ...
    def tick(self):
        ...
        for player in range(PLAYERS):
            ...
            player_move = self.player_controllers[player].tick(
                {"map": player_map, "ID": player, "tick": self.current_tick})
