...
class Game:
    ...
    # This function contains a lot of tricks to reduce the data sent, in an effort to increase performances
    async def async_tick(self):
        ...
        for player in range(PLAYERS):
            ...
            if(self.current_tick == 0 or self.last_tick_deads):
                ts.append(self.player_controllers[player].tick_map(
                    self.current_tick))  # Send map to every player
            else:
                ts.append(self.player_controllers[player].tick(
                    self.current_tick))  # Send updates to every player
        if(self.last_tick_deads):
            self.last_tick_deads = False
        ...
        # Update every map and return updated tiles
    ...
    def generate_results(self, path):
        ...
        for x in range(PLAYERS):
            ...
            tot_power = sum([y.get_current_value() if y.get_owner_ID(
            ) == x else 0 for y in self.map.hash_map.values()])
